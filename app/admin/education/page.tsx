        ...
        <div className="space-y-3">
          {sorted.map((edu, i) => (
            <motion.div key={edu.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="w-10 h-10 bg-violet-500/10 border border-violet-500/20 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                <FiBookOpen size={15} className="text-violet-400" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <p className="text-white font-semibold">{edu.degree}</p>
                  {edu.current && (
                    <span className="text-xs text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 rounded-full px-2 py-0.5">En cours</span>
                  )}
                </div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-3 cursor-pointer">
                <div onClick={() => setForm({ ...form, current: !form.current })}
                  className={`relative w-10 h-5 rounded-full transition-colors ${form.current ? "bg-emerald-500" : "bg-[#2a2a2a]"}`}>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              ...
              className="w-full bg-[#161616] border border-[#252525] focus:border-emerald-500/50 text-white placeholder-slate-600 rounded-xl px-4 py-2.5 text-sm outline-none resize-none transition-colors" />
          </div>
